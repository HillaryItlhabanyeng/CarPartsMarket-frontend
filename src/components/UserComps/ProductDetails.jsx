...
import { getProductFitment } from "../../lib/fitment";
...
function ProductDetails({ data }) {
  ...
  function addToCarthandler(id) {
    ...
    if (isItemExists) {
      ...
      if (data.stock < 1) return toast.error("This car part is out of stock!");
      else {
  ...
  const fitment = getProductFitment(data);
          ...
          >
            ...
            <div className="mt-4 rounded-2xl border border-success-100 bg-success-50/70 p-4">
              <div className="flex items-start gap-3">
                <span className="mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white text-success-600 shadow-sm">✓</span>
                <div className="min-w-0">
                  <p className="text-[13px] font-bold text-success-700">Vehicle compatibility</p>
                  <p className="mt-1 text-[13px] leading-relaxed text-ink-600">
                    {fitment.make || fitment.model || fitment.year
                      ? `Fits ${[fitment.make, fitment.model, fitment.year].filter(Boolean).join(" ")}${fitment.engine ? ` — ${fitment.engine}` : ""}.`
                      : "This seller has not supplied vehicle fitment yet. Check compatibility before ordering."}
                  </p>
                </div>
              </div>
            </div>
