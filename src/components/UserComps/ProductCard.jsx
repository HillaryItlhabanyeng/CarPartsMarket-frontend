...
import { getProductFitment, getSelectedVehicle, productFitsVehicle } from "../../lib/fitment";
...
function ProductCard({ product, isEvent }) {
  ...
  const fitsSelectedVehicle = !selectedVehicle || productFitsVehicle(product, selectedVehicle);
      ...
      >
        ...
        <div className="relative overflow-hidden bg-ink-50/70 p-4">
          ...
          <div className="pointer-events-none absolute left-3 top-3 flex flex-col gap-1.5">
            ...
            {fitsSelectedVehicle && selectedVehicle && (
              <span className="rounded-full bg-success-500 px-2.5 py-1 text-[11px] font-bold text-white shadow-sm">
                Fits your vehicle
              </span>
            )}
            {outOfStock && (
        ...
        <div className="flex flex-1 flex-col px-4 pb-4">
          ...
          >
            {product?.shop?.name || "Marketplace Seller"}
          </Link>
          ...
          {(fitment.make || fitment.model || fitment.year) && (
            <p className="mt-2 line-clamp-1 text-[12px] font-medium text-success-700">
              Fits: {[fitment.make, fitment.model, fitment.year].filter(Boolean).join(" ")}
            </p>
          )}
